...
the word in question must be a literal of some sort, capitalize the lteral as you would in
its natural context. For example, if your list item starts with a snippet of code, you
should not change the code's capitalization. (See
<<spring-style-guide-source-code-listings>>.)
...
paragraph level, Often, it is easy to overuse the technique, but it is worthwhile
